...
Usage:
    python -m merge.scripts.eval.evaluate_checkpoints --config /path/to/config.yaml
    python -m merge.scripts.eval.evaluate_checkpoints --config /path/to/config.yaml --ckpt-dir /path/to/ckpts
    python -m merge.scripts.eval.evaluate_checkpoints --config cfg.yaml --run-tag-auto --no-plots
      # ->
...
from merge.model import DiffusionPolicy
from merge.datasets import compute_global_action_stats
from libero.libero.benchmark import get_benchmark
from merge.scripts.eval.rollout_evaluator import evaluate_policy_on_task
from merge.scripts.eval.cl_metrics import (
    compute_nbt,
    compute_average_sr,
...
def evaluate_all_checkpoints(
    ...
    if resume:
        if eval_log_path.exists():
            ...
            completed_checkpoints = {entry.get("checkpoint") for entry in eval_log}
            completed_checkpoints.discard(None)
            print(f"[resume] loaded {len(completed_checkpoints)} completed checkpoint(s)")
        if perf_matrix_path.exists():
            perf_matrix = np.load(perf_matrix_path)
            print(f"[resume] loaded intermediate performance matrix: {perf_matrix_path}")
        elif (results_dir / "perf_matrix.npy").exists():
            perf_matrix = np.load(results_dir / "perf_matrix.npy")
            print(f"[resume] loaded final performance matrix: {results_dir / 'perf_matrix.npy'}")

    total_start = time.time()
    max_task_k_seen = max(
        [int(entry.get("task_idx", -1)) for entry in eval_log] or [-1]
    )

    for ckpt_path in ckpt_files:
        if resume and ckpt_path.name in completed_checkpoints:
            print(f"\n[resume] Skipping completed checkpoint: {ckpt_path.name}")
            continue
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--resume",
        action="store_true",
        help="Resume from existing eval_log.json/perf_matrix_intermediate.npy in the results directory.",
    )
