# -*- coding: utf-8 -*-
"""
Diffusion Policy training on LIBERO-90 with full-dataset shuffled epochs.
...
def train(cfg):
    ...
    for epoch in range(train_cfg["num_epochs"]):
        ...
        for batch in pbar:
            ...
            pbar.set_postfix(
                loss=f"{loss_val:.4f}", lr=f"{scheduler.get_last_lr()[0]:.2e}"
            )
        ...
        if tb_writer:
            ...
            tb_writer.add_scalar("epoch/lr", scheduler.get_last_lr()[0], epoch + 1)
        ...
        if (epoch + 1) % log_cfg.get("save_interval", 10) == 0:
            ...
            print(f"Saved EMA checkpoint: {ema_ckpt_path}")

        if avg_loss < best_loss:
            best_loss = avg_loss
            best_path = ckpt_dir / "best.pt"
            best_ema_path = ckpt_dir / "best_ema.pt"
            _save_checkpoint(best_path, checkpoint_payload, model, ema, False)
            _save_checkpoint(best_ema_path, checkpoint_payload, model, ema, True)
