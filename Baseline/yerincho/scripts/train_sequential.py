...
def _save_checkpoint(
    ...
    checkpoint["model_state_dict"] = (
        ema.state_dict() if use_ema_weights else model.state_dict()
    )
...
def verify_data_files(data_root: str, benchmark):
    ...
    if missing:
        ...
        raise FileNotFoundError(
            ...
            + f"\n\nPlease download the required dataset for benchmark '{benchmark_name}' first."
        )
...
def train_on_task(
    ...
    for epoch in range(epochs):
        ...
        for batch in pbar:
            ...
            if tb_writer and global_step % log_cfg.get("log_interval", 50) == 0:
                ...
                tb_writer.add_scalar(
                    f"task_{task_idx}/train/lr", scheduler.get_last_lr()[0], tb_step
                )
        ...
        if tb_writer:
            ...
            tb_writer.add_scalar(
                f"task_{task_idx}/epoch/lr", scheduler.get_last_lr()[0], epoch + 1
            )
...
def main(cfg, skip_eval=False):
    ...
    for task_k in range(n_tasks):
        ...
        if tb_writer:
            tb_writer.add_scalar(
                f"task_{task_k}/summary/train_time_s", train_time, task_k + 1
            )
            tb_writer.add_scalar(
                f"task_{task_k}/summary/final_train_loss",
                epoch_losses[-1],
                task_k + 1,
            )
        ...
        if not skip_eval:
            ...
            eval_results = evaluate_checkpoint_on_all_tasks(
                ...
                video_fps=eval_cfg.get("video_fps", 20),
                video_episodes_to_save=eval_cfg.get("video_episodes_to_save", 1),
                video_camera_key=eval_cfg.get("video_camera_key", "agentview_image"),
            )
            ...
            if tb_writer:
                ...
                tb_writer.add_scalar(
                    f"task_{task_k}/summary/eval_time_s", eval_time, task_k + 1
                )
    ...
    if skip_eval:
        ...
        print("  Run evaluate_checkpoints.py to compute metrics.\n")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--skip-eval",
        action="store_true",
        help="Skip rollout evaluation during training; only save checkpoints.",
    )
