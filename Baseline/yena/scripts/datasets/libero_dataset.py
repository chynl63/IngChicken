"""
LIBERO-90 Dataset with per-task uniform sampling for Diffusion Policy.

Sampling strategy:
  1. Uniformly sample a task (from 90 tasks)
  2. Uniformly sample an episode from that task
  3. Uniformly sample a starting timestep from that episode
  4. Extract (obs_horizon, action_horizon) chunk

This ensures every task contributes equally regardless of episode count or length.
"""
...
class LiberoUniformDataset(Dataset):
    ...
    def __init__(
        ...
        if obs_keys is None:
            obs_keys = ["agentview_image", "robot0_eef_pos", "robot0_eef_quat", "robot0_gripper_qpos"]
...
if __name__ == "__main__":
    ...
    print(f"  min={task_counts.min():.0f}, max={task_counts.max():.0f}, "
