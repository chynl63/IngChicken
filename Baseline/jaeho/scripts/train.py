...
def train(cfg):
    ...
    for epoch in range(train_cfg["num_epochs"]):
        ...
        if avg_loss < best_loss:
            ...
            _save_checkpoint(best_path, checkpoint_payload, model, ema, use_ema_weights=False)
            _save_checkpoint(best_ema_path, checkpoint_payload, model, ema, use_ema_weights=True)

    print(f"\nTraining complete! Best loss: {best_loss:.4f}")
    print(f"Checkpoints saved to: {ckpt_dir}")
    if tb_writer:
        tb_writer.add_scalar("train/best_loss", best_loss, global_step)
        tb_writer.flush()
        tb_writer.close()
