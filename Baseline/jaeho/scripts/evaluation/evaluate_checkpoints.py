...
def evaluate_all_checkpoints(
    ...
    device_str = os.environ.get(
        "EVAL_DEVICE",
        "cuda" if torch.cuda.is_available() else "cpu",
    )
    device = torch.device(device_str)
    print(f"Evaluation device: {device} (config device ignored)")
    ...
    if ckpt_pattern is None:
        ckpt_files = [path for path in ckpt_files if not path.stem.endswith("_ema")]
    if not ckpt_files:
    ...
    for ckpt_path in ckpt_files:
        ...
        for task_j in task_indices:
            ...
            sr, ep_results = evaluate_policy_on_task(
                ...
                video_dir=str(results_dir / "videos" / ckpt_path.stem),
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--no-plots",
        action="store_true",
        help="Skip heatmap/forgetting PNG generation for sharded evaluation jobs.",
    )
